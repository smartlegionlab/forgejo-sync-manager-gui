...
class DeleteWorker(QThread):
    ...
    def __init__(self, sync_manager: GUISyncManager, repositories: List[dict]):
        super().__init__()
    ...
    def run(self):
        results = {"deleted": 0, "failed": 0, "not_found": 0}
        ...
        for i, repo in enumerate(self.repositories, 1):
            ...
            try:
                if repo_path.exists():
                    ...
                    results["deleted"] += 1
                    self.progress.emit(i, total, repo_name, "DELETED")
                else:
                    results["not_found"] += 1
                    self.progress.emit(i, total, repo_name, "NOT_FOUND")
...
class DeleteAllDialog(QDialog):
    ...
    def __init__(self, sync_manager: GUISyncManager, repositories: List[dict], parent=None):
        super().__init__(parent)
        self.sync_manager = sync_manager
        self.repositories = repositories
        self.worker = None

        self.setWindowTitle("Delete All Repositories")
        self.setMinimumSize(700, 500)
        self.setModal(True)

        self.setup_ui()

        QTimer.singleShot(100, self.start_delete)
    ...
    def setup_ui(self):
        ...
        title = QLabel("DELETING ALL REPOSITORIES")
        title_font = QFont()
    ...
    def start_delete(self):
        self.worker = DeleteWorker(self.sync_manager, self.repositories)
    ...
    def on_progress(self, current: int, total: int, repo_name: str, status: str):
        ...
        if status == "DELETED":
            self.repo_status_updated.emit(repo_name, False)

    def on_finished(self, results: dict):
        self.current_label.setText("Deletion completed!")
